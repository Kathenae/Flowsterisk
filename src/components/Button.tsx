...
export default function Button({ children, primary, danger, full, className, draggable, onClick, onDragStart }: PropsWithChildren<ButtonProps>) {
  ...
  return (
    <button 
      className={`p-2 bg-white !dark:text-gray-100 dark:bg-dark-200 hover:bg-brand-200 hover:border-brand-200 border-gray-800 border-1 rounded-lg ${full && "w-full"} ${primary && primaryClasses} ${danger && dangerClasses} ${className}`} 
      draggable={draggable} 
      onClick={onClick}
